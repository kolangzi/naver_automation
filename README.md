...
4. **DRY-RUN**: 체크 시 실제 등록 없이 요소 탐색/입력 필드 검증만 수행
5. **[시작]** 버튼 클릭 → 브라우저에서 비밀번호 입력 후 로그인 (3분 이내)

### 완료 후 액션

시작 버튼 옆의 "완료 후" 드롭다운으로 작업 완료 시 동작을 선택할 수 있습니다.

- **없음**: 기본값. 완료 후 대기 상태로 유지
- **프로그램 종료**: 작업 완료 후 프로그램 자동 종료
- **잠자기 모드**: 작업 완료 후 프로그램 종료 및 Mac 잠자기 모드 진입
...
- 기능 2에서 이웃 글 방문 시 본문 길이에 비례한 체류 시간이 자동으로 적용됩니다
